...
def min_tr_WA(k, A):
    ...
    constraints = [
        ...
        cp.diag(
            W[2*k:4*k, 2*k:4*k]) - 10*W[2*k:4*k, 4*k] + 9*np.ones((2*k)) == 0,
        W[4*k, 4*k] == 1]

    # formulate and solve problem
    prob = cp.Problem(objective, constraints)
    prob.solve(solver=cp.MOSEK, verbose=True, parallel=True)
    # prob.solve(solver=cp.SCS, verbose=True, parallel=True, max_iters=50000)
